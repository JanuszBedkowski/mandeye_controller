...
static std::vector<uint8_t> serializePointCloud2(uint64_t timestamp_ns, const LidarPointsBuffer& pts, const std::string& frame_id)
{
	CdrWriter w;
...
static std::vector<uint8_t> serializeImu(uint64_t timestamp_ns, const LidarIMU& imu, const std::string& frame_id)
{
	CdrWriter w;
	writeHeader(w, timestamp_ns, frame_id);
