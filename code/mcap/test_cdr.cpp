...
static constexpr uint64_t kImuStampNs = static_cast<uint64_t>(kImuStampSec) * 1'000'000'000ULL + kImuStampNsec;
...
static bool checkPc2Cdr(const uint8_t* buf, size_t size, const char* label)
{
	...
	if(raw)
	{
		...
		for(uint32_t i = 0; i < kNPts; ++i)
		{
			...
			CHECK(deq(rel, expRel), "pt[" << i << "].rel_t got=" << rel << " want=" << expRel);
		}
	}
...
static bool checkImuCdr(const uint8_t* buf, size_t size, const char* label)
{
	...
	const double ox = r.read_f64(), oy = r.read_f64(), oz = r.read_f64(), ow = r.read_f64();
...
static std::vector<uint8_t> serializeTestImu()
{
	...
	// linear_acceleration
	w.write_f64(kAccel[0]);
	w.write_f64(kAccel[1]);
	w.write_f64(kAccel[2]);
...
static int
run(const char* name, bool (*check)(const uint8_t*, size_t, const char*), const uint8_t* gold, size_t goldSz, std::vector<uint8_t> (*serialize)())
{
	int result = 0;
	std::cout << name << " — gold decode: ";
	if(!check(gold, goldSz, "gold"))
	{
		std::cout << "FAIL\n";
		result = 1;
	}
	...
	if(!check(ours.data(), ours.size(), "ours"))
	{
		std::cout << "FAIL\n";
		result = 1;
	}
