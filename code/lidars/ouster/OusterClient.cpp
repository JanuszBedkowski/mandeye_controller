...
OusterClient::OusterClient()
	: m_impl(std::make_unique<OusterClientImpl>())
{ }
...
nlohmann::json OusterClient::produceStatus()
{
	...
	// Timestamp information
	{
		...
		status["session_start_s"] = m_impl->m_sessionStart.has_value() ? double(*m_impl->m_sessionStart) / 1e9 : -1.0;
	...
	try
	{
		...
		for(const auto& info : m_sensorInfos)
		{
			...
			std::cerr << "  Firmware version:  " << info.image_rev << "\n  Serial number:     " << info.sn
					  ...
					  << column_window.first << ", " << column_window.second << "]" << std::endl;
		}
		...
		for(size_t i = 0; i < m_sensors.size(); i++)
		{
			...
			if(intrinsicJson.contains("lidar_to_sensor_transform") && intrinsicJson["lidar_to_sensor_transform"].is_array())
			{
				...
				//clang-format off
				transform << arr[0], arr[1], arr[2], arr[3], arr[4], arr[5], arr[6], arr[7], arr[8], arr[9], arr[10], arr[11], arr[12], arr[13],
					arr[14], arr[15];
				//clang-format on
