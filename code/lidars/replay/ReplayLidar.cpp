...
double ReplayLidar::getTimestamp()       { return m_currentTimestamp.load(); }
double ReplayLidar::getSessionDuration() { return m_currentTimestamp.load() - m_sessionStart; }
double ReplayLidar::getSessionStart()    { return m_sessionStart; }
void   ReplayLidar::initializeDuration() { m_sessionStart = m_currentTimestamp.load(); }
...
LidarPointsBufferPtr ReplayLidar::loadLaz(const std::filesystem::path& path)
{
    ...
    for(laszip_I64 i = 0; i < num_points; ++i)
    {
        ...
        if(laszip_get_coordinates(reader, coords)) break;
...
LidarIMUBufferPtr ReplayLidar::loadCsv(const std::filesystem::path& path)
{
    ...
    while(std::getline(file, line))
    {
        ...
        LidarIMU imu{};
        ss >> imu.timestamp >> imu.gyro_x >> imu.gyro_y >> imu.gyro_z
           >> imu.acc_x >> imu.acc_y >> imu.acc_z >> imu.laser_id >> imu.epoch_time;
        if(!ss.fail()) buffer->push_back(imu);
    }
    return buffer;
}
...
std::filesystem::path ReplayLidar::convertToMcap(const std::filesystem::path& dir)
{
    ...
    for(int idx : chunks)
    {
        ...
        std::cout << "  chunk " << idx << " → " << pts->size() << " pts, "
                  << imu->size() << " imu\n";
    }
        ...
        for(auto it = view.begin(); !m_isDone && it != view.end(); ++it)
        {
            ...
            if(topic == "/lidar_points")
            {
                ...
                if(pts && !pts->empty())
                {
                    std::lock_guard<std::mutex> lock(m_bufferMutex);
                    if(m_bufferLidarPtr)
                        m_bufferLidarPtr->insert(m_bufferLidarPtr->end(),
                                                  pts->begin(), pts->end());
                }
