...
struct ScanStats
{
	...
	FileGroupStats extra_gnss;  // one per LEPTON* dir, name trimmed
...
static std::string formatSize(double megabytes)
{
	std::ostringstream ss;
	if(megabytes >= 1024.0)
		ss << std::fixed << std::setprecision(1) << (megabytes / 1024.0) << "GB";
	else
		ss << std::fixed << std::setprecision(0) << megabytes << "MB";
	return ss.str();
}
...
ScanStats scanDirectory(const std::string& path)
{
	...
	for(const auto& entry : fs::directory_iterator(path))
	{
		...
		else if(entry.is_directory())
		{
			...
			else if (dirname.rfind("EXTRA_GNSS", 0) == 0)
			{
				FileGroupStats g;
				// GNSS_longname -> GNSS_ + first 4 chars of suffix
				stats.extra_gnss.label = "GNSS";
				accumulateDir(entry.path(), stats.extra_gnss);
			}
		}
	}
	...
	for (;;) {
		...
		// line3 cameras:
		{
			...
			for (int i = 0; i < stats.cameras.size(); i++) {
				ss << "C"<<i<<":"<<formatSize(stats.cameras[i].megabytes)<<" ";
			}
		...
		// line4 leptons:
		{
			...
			for (int i = 0; i < stats.leptons.size(); i++) {
				ss << "L"<<i<<":"<<formatSize(stats.leptons[i].megabytes)<<" ";
			}
		...
		{
			std::ostringstream ss;
			ss << "GNSS:" << formatSize(stats.extra_gnss.megabytes) << "(" << stats.extra_gnss.count << ")";
			u8g2.drawStr(1, Y4, ss.str().c_str());
		}
