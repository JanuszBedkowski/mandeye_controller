...
void NtripClient::do_connect()
{
	...
	resolver_.async_resolve(host_, port_, [this](const boost::system::error_code& ec, const auto& results) {
		if(ec)
		{
		...
		boost::asio::async_connect(socket_, results, [this](const boost::system::error_code& ec, const auto&) {
			if(ec)
			{
...
void NtripClient::send_request()
{
	...
	boost::asio::async_write(socket_, boost::asio::buffer(request), [this](const boost::system::error_code& ec, std::size_t) {
		if(ec)
		{
...
void NtripClient::read_stream()
{
	boost::asio::async_read(
		socket_, response_, boost::asio::transfer_at_least(1), [this](const boost::system::error_code& ec, std::size_t bytes_transferred) {
			if(!ec && callback_)
			{
...
void NtripClient::send_gga(const std::string& gga_sentence)
{
	if(socket_.is_open())
	{
		...
		boost::asio::async_write(socket_, boost::asio::buffer(msg), [](const boost::system::error_code& ec, std::size_t) {
			if(ec)
			{
