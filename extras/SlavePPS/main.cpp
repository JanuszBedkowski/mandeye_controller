...
int main()
{
    ...
    const std::string uartPort  = getEnvString("SLAVE_PPS_UART_PORT",      mandeye::GetLidarSyncPorts().empty() ? "/dev/ttyAMA1" : mandeye::GetLidarSyncPorts()[0]);
    ...
    std::cout << "GPS+PPS sync started"
              << " uart=" << uartPort << " baud=" << baudInt
              << " pps=" << ppsDevice << std::endl;
